...
namespace Not_Virus
{
    class MovingPic2
    {
        ...
        public MovingPic2(int x, int y, int formWidth, int formHeight, int xSpeed, int ySpeed, Color color, int width, int height)
        {
            _x = x;
            _y = y;
            _xSpeed = xSpeed;
            ...
            var rand = new Random();
            var files = Directory.GetFiles(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures), "*.png");

            string a = files[rand.Next(files.Length)];

            try
            {
                _img = Image.FromFile(a);
            } catch
            {
                _img = Image.FromFile("images\\wario.png");
            }
        }
